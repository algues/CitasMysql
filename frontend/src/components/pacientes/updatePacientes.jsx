...
const ModificarPacientes = () => {
    ...
    const handleClick = async e =>{
        e.preventDefault()
        try{
           await axios.put("http://localhost:8800/pacientes/"+ pacIdentificacion, paciente)
           .then(res =>{
            if(res.data === "Success"){
        ...
        <div>
            ...
            {paciente && (
                <div className='container'>
                    ...
                    <input type='text' name='pacApellidos' onChange={handleChange} value={paciente.pacApellidos}></input>
                    <label>Nombres:</label>
                    <input type='text' name='pacNombres' onChange={handleChange} value={paciente.pacNombres}></input>
                    <label>Fecha de Nacimiento:</label>
                    <input type='text' name='pacFechaNacimiento' onChange={handleChange} value={paciente.pacFechaNacimiento}></input>
                    <label>Telefono:</label>
                    <input type='text' name='pacTelefono' onChange={handleChange} value={paciente.pacTelefono}></input>
                    <label>Sexo:</label>
                    <input type='text' name='pacSexo' onChange={handleChange} value={paciente.pacSexo}></input>   
